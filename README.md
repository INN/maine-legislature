Copyright 2015-2017 INN.  All rights reserved.  No part of these materials may be reproduced, modified, stored in a retrieval system, or retransmitted, in any form or by any means, electronic, mechanical or otherwise, without prior written permission from INN.
...
**TKTK: Describe maine-legislature here.**
...
```
cd maine-legislature
mkvirtualenv maine-legislature
...
Large media assets (images, videos, audio) are synced with an Amazon S3 bucket specified in ``app_config.ASSETS_S3_BUCKET`` in a folder with the name of the project. (This bucket should not be the same as any of your ``app_config.PRODUCTION_S3_BUCKET`` or ``app_config.STAGING_S3_BUCKETS``.) This allows everyone who works on the project to access these assets without storing them in the repo, giving us faster clone times and the ability to open source our work.
...
```
* * * * * ubuntu bash /home/ubuntu/apps/maine_legislature/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test
```
...
|--------|------|-----|-----|
|maine-legislature|tweet|`location`||
|maine-legislature|facebook|`location`||
|maine-legislature|email|`location`||
|maine-legislature|new-comment||
|maine-legislature|open-share-discuss||
|maine-legislature|close-share-discuss||
|maine-legislature|summary-copied||
|maine-legislature|featured-tweet-action|`action`|
|maine-legislature|featured-facebook-action|`action`|
